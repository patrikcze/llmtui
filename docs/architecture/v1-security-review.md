...
## 8. 2026-09-20 finding: `personalapps.Service` authorization could go stale behind the adapter queue

| Finding | Severity | Status | Fix |
| --- | --- | --- | --- |
| `Service.Execute` (`internal/personalapps/service.go`) snapshotted `scope`/`conn` once, before queuing for `s.adapter` (the mutex that serializes all Mail/Calendar work), and handed that same pre-queue snapshot straight through to the read handlers and to `changeApply`'s own "re-check policy and scope now, not only at prepare time" loop. `SetScope`/`Disconnect` take only `s.mu`, not `s.adapter`, so a human revoking access while an earlier call held `s.adapter` did not affect a call already queued behind it: by the time the queued call was finally dispatched, it still carried the authorization that held *before* it started waiting, and `changeApply`'s "re-check" compared against that same stale value rather than anything current. | Medium (CWE-862/CWE-363-adjacent: stale authorization across a lock wait) | Fixed (partial — see limitation below) | `internal/personalapps/service.go` (`Execute`): scope/conn are re-read from `s.mu` and re-checked immediately after `s.adapter.Lock()` succeeds, and that live snapshot — not the pre-queue one — is what every handler and `changeApply`'s internal loop receive. Two regression tests in `internal/personalapps/security_review_2026_09_20_test.go` hold `s.adapter` themselves to force a call to queue, revoke authorization while it waits, then release the lock: `TestQueuedReadIsRevalidatedAgainstLiveAuthorizationAfterAdapterLock` (a plain read) and `TestQueuedChangeApplyIsRevalidatedAgainstLiveScopeAfterAdapterLock` (isolates `changeApply`'s own re-check from the plan store's unrelated `Reset()` side effect on `Disconnect`/`SetScope`, which already independently invalidates a queued `change_apply`'s plan lookup — see the second test's doc comment). Both verified to fail on the pre-fix code and pass on the fixed code. |

**Limitation, by design, not attempted here:** this fix closes the "stale snapshot survives the queue wait" gap but does not add cancellation for external adapter work already in progress at the moment of revocation — a network call to Mail/Calendar's companion process that had already started when `Disconnect` fires runs to completion. The recommended remediation's "revocation generations with cancellation" is a larger architectural change (threading cancellation through every adapter implementation, an epoch counter checked mid-call) that was judged out of scope for this fix; `docs/personal-apps.md` now states this limitation explicitly. `SetScope`/`Disconnect` intentionally continue to not block on `s.adapter` — making them synchronous with in-flight adapter work would freeze the calling goroutine (the TUI's `Update`, which must never block) for the duration of that work.
