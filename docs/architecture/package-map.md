...
| `internal/eval` | Developer-only, opt-in evaluation library and endpoint tests. Repeats contract-stage and safe native-tool conformance probes, records bounded content-safe metrics, and writes JSONL reports. It never runs automatically in CI, never executes host tools, and is not part of the normal chat runtime. | `doc.go`, `live.go`; opt-in endpoint test `live_test.go`; fake-provider tests `live_test_internal_test.go` |
