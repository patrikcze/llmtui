...
| `internal/personalapps` | The optional Apple Mail / Apple Calendar domain: the closed `personal_apps` operation vocabulary, strict shared decoder for the native and fenced protocols, opaque session handles over host-side resource refs, the user-configured `Scope` (empty allowlist authorizes nothing), half-open interval / DST / free-slot logic, and the immutable prepare-approve-apply plan lifecycle. Adapters (`MailBackend`, `CalendarBackend`, `Mutator`, `Navigator`, `Journal`) are injected interfaces, so nothing here launches an app, starts a helper or requests an OS permission. **Not wired into `tools` or `tui` yet** — the native adapters, the durable operation ledger and the approval UI are later slices. Must never import `tui`, `provider`, `config` or `tools`. | `personalapps.go`, `request.go`, `changes.go`, `plan.go`, `policy.go`, `service.go`, `timewindow.go` |
