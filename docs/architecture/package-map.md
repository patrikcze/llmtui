...
| `internal/runtime` (top-level, *not* Go's `runtime`) | Resolution / verification / install of the llama.cpp **shared libraries** for the embedded provider. Multi-tier lookup (config → env → exe-relative → user-data → legacy), hash verification against embedded trusted digests, `install.go` downloads and pins managed runtimes, `manifest.go`/`pin.go` track them, and platform-specific ownership/preparation handles release layout. See also `docs/architecture/embedded-local-inference.md`. | `resolver.go`, `verify.go`, `install.go`, `manifest.go`, `pin.go`, `platform.go`, `ownership_{unix,windows}.go`, `prepare_{other,windows}.go` |
| `internal/selfupdate` | Self-management for the **llmtui binary itself** (`self check/update/install/path`) — distinct from `internal/runtime`, which manages the llama.cpp libraries. GitHub Releases discovery + semver comparison (`github.go`, `version.go`), asset selection/checking (`asset.go`, `check.go`), streamed HTTPS download with a host allowlist (`download.go`), `checksums.txt` verification (`checksum.go`), hardened tar.gz/zip extraction into a private staging dir (`archive.go`), install-path layout per OS/scope (`layout*.go`), and a transactional binary+runtime replacement with rollback (`install.go`). Cobra-free so the transaction is unit-testable. Imports `internal/runtime` only to locate a bundled runtime to copy. See `docs/self-management.md`. | `github.go`, `version.go`, `asset.go`, `check.go`, `download.go`, `checksum.go`, `archive.go`, `layout*.go`, `manifest.go`, `pathenv.go`, `pathinfo.go`, `pathpublic.go`, `install.go`, `installcurrent.go`, `update.go`, `selfupdate.go` |
...
| `internal/entity` | Bounded session-local runtime entity registry with opaque IDs, progressive detail resolution, deterministic eviction, expiration, retention scopes, and optional owner-only temporary disk backing. See `docs/architecture/entity-context-runtime.md` and the [Phase 8 calibration report](next-generation-tool-runtime-phase8-report.md). | `entity.go`, `registry.go`, `resource.go`, `body_registry.go`, `body_memory.go`, `body_disk.go` |
...
| `internal/personalapps` | The optional Apple Mail / Apple Calendar domain: the closed `personal_apps` operation vocabulary, strict shared decoder for the native and fenced protocols, opaque session handles over host-side resource refs, the user-configured `Scope` (empty allowlist authorizes nothing), half-open interval / DST / free-slot logic, and the immutable prepare-approve-apply plan lifecycle. Adapters (`MailBackend`, `CalendarBackend`, `Mutator`, `Navigator`, `Journal`) are injected interfaces, so nothing here launches an app, starts a helper or requests an OS permission by itself. Wired into `tools`/`tui` as twelve flat native tools and one fenced form per operation; both routes reassemble the same `{"operation","arguments"}` envelope `ParseRequest` requires. Mail has a real darwin adapter (`mail_darwin.go`, `mail_bridge.go`, `mail_mutator.go`) behind a fixed, embedded JXA script over `osascript`; Calendar has a real adapter (`calendar_darwin.go`, `calendar_bridge.go`, `calendar_mutator.go`) behind a separately configured EventKit companion (`native/personal-apps-calendar/`). Every mutation re-reads its target immediately before applying and compares it against a Go-computed fingerprint recorded at last read; a mismatch refuses the item rather than applying to stale state. `mutator.go` dispatches a resolved change to the Mail or Calendar mutator by `Change.Adapter()`. Must never import `tui`, `provider`, `config` or `tools`. | `personalapps.go`, `approvals.go`, `backend.go`, `request.go`, `decode.go`, `changes.go`, `handles.go`, `plan.go`, `policy.go`, `service.go`, `diagnostics.go`, `limits.go`, `mail_bridge.go`, `mail_darwin.go`, `mail_mutator.go`, `calendar_bridge.go`, `calendar_darwin.go`, `calendar_mutator.go`, `mutator.go`, `mutation_ledger*.go`, `platform_*.go`, `*_other.go` |
...
| `internal/procutil` | Subprocess-group management so wrapper commands cannot orphan grandchildren. Unix / Windows split. | `procutil.go`, `proc_unix.go`, `proc_windows.go` |
...
| `internal/selfupdate` | Manages the **llmtui executable** (download/verify/replace from GitHub Releases), not the llama.cpp libraries. Depends on `internal/runtime` only to find a bundled runtime to copy during `self install`. |
