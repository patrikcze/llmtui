...
| `internal/personalapps` | The optional Apple Mail / Apple Calendar domain: the closed `personal_apps` operation vocabulary, strict shared decoder for the native and fenced protocols, opaque session handles over host-side resource refs, the user-configured `Scope` (empty allowlist authorizes nothing), half-open interval / DST / free-slot logic, and the immutable prepare-approve-apply plan lifecycle. Adapters (`MailBackend`, `CalendarBackend`, `Mutator`, `Navigator`, `Journal`) are injected interfaces, so nothing here launches an app, starts a helper or requests an OS permission by itself. Wired into `tools`/`tui` as the `personal_apps` tool. Mail has a real darwin adapter (`mail_darwin.go`, `mail_bridge.go`) behind a fixed, embedded JXA script invoked over `osascript` — read-only (`mail_accounts`/`mail_mailboxes`/`mail_search`/`mail_read`); mailbox identity is a resolved `(account UUID, disambiguated path)` pair, since Mail's scripting dictionary exposes no stable mailbox ID and allows same-named sibling mailboxes. **Calendar still has no adapter, and Mail has no mutation adapter** — both report `unsupported_operation`; the EventKit companion and the durable operation ledger are later slices. Must never import `tui`, `provider`, `config` or `tools`. | `personalapps.go`, `request.go`, `changes.go`, `plan.go`, `policy.go`, `service.go`, `timewindow.go`, `mail_bridge.go`, `mail_darwin.go` |
