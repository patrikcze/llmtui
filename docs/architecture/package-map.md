...
| `internal/cli` | The Cobra command tree: `chat`, `config`, `providers`, `models`, `runtime`, `doctor`, `history`, `stats`, `version`, `self`. `root.go` owns flag → viper → config precedence (only binds flags the user actually set). Each other file is one subcommand. `self.go` (like `version.go`) bypasses config loading. | `root.go`, `chat.go`, `config.go`, `providers.go`, `models.go`, `runtime.go`, `doctor.go`, `history.go`, `stats.go`, `version.go`, `self.go` |
...
| `internal/personalapps` | The optional Apple Mail / Apple Calendar domain: the closed `personal_apps` operation vocabulary, strict shared decoder for the native and fenced protocols, opaque session handles over host-side resource refs, the user-configured `Scope` (empty allowlist authorizes nothing), half-open interval / DST / free-slot logic, and the immutable prepare-approve-apply plan lifecycle. Adapters (`MailBackend`, `CalendarBackend`, `Mutator`, `Navigator`, `Journal`) are injected interfaces, so nothing here launches an app, starts a helper or requests an OS permission by itself. Wired into `tools`/`tui` as twelve flat native tools (one per operation) plus the fenced `personal_apps` protocol; both reassemble the same `{"operation","arguments"}` envelope `ParseRequest` requires. Mail has a real darwin adapter (`mail_darwin.go`, `mail_bridge.go`, `mail_mutator.go`) behind a fixed, embedded JXA script over `osascript` — reads (`mail_accounts`/`mail_mailboxes`/`mail_search`/`mail_read`) and mutations (`mail_move`/`mail_set_read`/`mail_set_flag`/`mail_save_draft`); mailbox identity is a resolved `(account UUID, disambiguated path)` pair, since Mail's scripting dictionary exposes no stable mailbox ID and allows same-named sibling mailboxes. Calendar has a real adapter (`calendar_darwin.go`, `calendar_bridge.go`, `calendar_mutator.go`) behind a separately installed, signed EventKit companion (`native/personal-apps-calendar/`) over a length-framed JSON protocol — reads and `calendar_create_event`/`calendar_update_event`, gated on `calendar.helper_path` being configured. Every mutation re-reads its target immediately before applying and compares it against a Go-computed fingerprint recorded at last read; a mismatch refuses the item rather than applying to stale state. `mutator.go`'s `combinedMutator` dispatches a resolved change to the Mail or Calendar mutator by `Change.Adapter()`. Must never import `tui`, `provider`, `config` or `tools`. | `personalapps.go`, `request.go`, `changes.go`, `plan.go`, `policy.go`, `service.go`, `timewindow.go`, `mail_bridge.go`, `mail_darwin.go`, `mail_mutator.go`, `calendar_bridge.go`, `calendar_darwin.go`, `calendar_mutator.go`, `mutator.go`, `mutation_ledger.go` |
