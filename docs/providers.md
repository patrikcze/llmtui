...
- **Reasoning models** (that "think" before answering) stream their thinking
  ...
  never times out) and shows it as muted gray thought text. Use `/thoughts
  show|hide` to expand or collapse it without changing model behavior.
  Thinking is not part of the answer and is not saved, cached, or sent back
  as history.
...
- Strips a leaked leading `<think>…</think>` block out of the answer
  ...
  reasoning can be shown as UI-only thought text but is never stored in
